...
'''        {
...

        {
            ...
            "search_url_template": "https://www.allegro.pl/listing?string={query}"
        },
        {
            "name": "eBay",
    ...
    # User-defined path where CSV files will be saved
    save_path = r"C:\dropshipping\products_csv"

    # Loop through each shopping site and scrape products for each product in the list
    for site_info in shopping_sites:
        ...
        for category, products in categories_amazon_products.items():
            ...
            for product in products:
                ...
                search_url = site_info["search_url_template"].format(query=product.replace(" ", "+"))

                # Use scraper's methods directly for scraping and collecting
                scraper.open_home_page_and_navigate_to_product(site_info["home_url"], search_url)
                soup = scraper.extract_page_structure()
                scraper.detect_product_blocks(soup)

                # Calculate the number of products scraped for this product
                product_product_count = scraper.product_count - product_start_count
                print(f"Found {product_product_count} new products for the product: {product}")
        ...
        # Save collected products to a CSV
        csv_filename = os.path.join(save_path, f"{site_name.lower()}_scraped_products.csv")
